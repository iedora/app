...
17. **Components carry `data-test-id`.** Interactive elements (buttons, links, list items, table rows, cards, dialog/sheet roots, status pills, the trigger of any compound widget like Combobox / DropdownMenu / Tabs) MUST expose a `data-test-id`. Specs target by intent (`page.getByTestId('qr-codes-create-button')`), not by text content or CSS class — text drifts with i18n + copy edits, classes drift with Tailwind refactors, `data-test-id` is binding. Convention: `<slice>-<role>[-<modifier>]`; collections use `<slice>-<role>-{stableId}` so per-item assertions are unambiguous (e.g. `data-test-id="menu-builder-item-row-{itemId}"`). Form inputs may rely on the existing `id` + `htmlFor` pair — adding `data-test-id` is harmless but optional, since the label-link already provides a stable selector. `@iedora/design-system` primitives forward `data-test-id` to their root element via the standard prop-spread; never re-declare the prop on the consumer side. Playwright is wired with `testIdAttribute: 'data-test-id'` (see `playwright.config.ts`) so `getByTestId()` resolves the hyphenated form directly.
